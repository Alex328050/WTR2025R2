 ...
 * @LastEditors: Alex
 * @LastEditTime: 2025-06-08 11:41:23
 */
    ...
    for(;;)
    {
        if (flag_initComplete == 1)
        {
            ...
            if (throwhsm.wholestate == WHOLE_THROW)
            {
                ...
                if (throwhsm.throwstate == THROW_THROWOUT && (hDJI[1].Calculate.RotorRound < 1 || caldata.angle >= 32000 || caldata.angle <= 1000))  throwhsm.throwstate = THROW_BACK;
            ...
            else if (throwhsm.wholestate == WHOLE_CONTHROW)
            {
                ...
                if (throwhsm.throwstate == THROW_ACCELERATE && caldata.angle>=21500.0)  throwhsm.throwstate = THROW_THROWOUT;
                if (throwhsm.throwstate == THROW_THROWOUT && (hDJI[1].Calculate.RotorRound < PAWL_ROTATEDETECT || caldata.angle >= 32000 || caldata.angle <= 1000))  throwhsm.throwstate = THROW_BACK;
            }
            //拍球状态修改
            else if (throwhsm.wholestate == WHOLE_BOUNCE)
            {
                if      (throwhsm.bouncestate == BOUNCE_IDLE && rmctl.rmctl_msg.btn_Btn0 == 1)  throwhsm.bouncestate = BOUNCE_CATCHING;
                        ...
                        rmctl.rmctl_msg.btn_LeftCrossDown ==1
                        )
                {
            ...
            else if (throwhsm.wholestate == WHOLE_CONBOUNCE)
            {
                ...
                else if (throwhsm.bouncestate == BOUNCE_BOUNCE && rmctl.rmctl_msg.btn_LeftCrossUp ==1)
                {
                    throwhsm.bouncestate = BOUNCE_RESERVEMOTOR;
                }
                else if ((throwhsm.bouncestate == BOUNCE_RESERVEMOTOR || throwhsm.bouncestate == BOUNCE_BOUNCE) && rmctl.rmctl_msg.btn_LeftCrossDown == 1)
                {
                    throwhsm.bouncestate = BOUNCE_CATCHANDADJUSTPOSTURE;
                }
                else if (throwhsm.bouncestate == BOUNCE_CATCHANDADJUSTPOSTURE && rmctl.rmctl_msg.btn_Btn2 == 1)  throwhsm.bouncestate = BOUNCE_BOUNCE;
                else if (throwhsm.bouncestate == BOUNCE_CATCHANDADJUSTPOSTURE && rmctl.rmctl_msg.btn_Btn3 == 1)  throwhsm.bouncestate = BOUNCE_WAITBALL;
                else if (throwhsm.bouncestate == BOUNCE_WAITBALL && rmctl.rmctl_msg.btn_Btn4 == 1)  throwhsm.bouncestate = BOUNCE_CATCHANDADJUSTPOSTURE;
            }
        }
        osDelay(2);
    }
