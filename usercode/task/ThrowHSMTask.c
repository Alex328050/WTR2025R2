...
void ThrowHSMTask(void* argument)
{
    ...
    for(;;)
    {
        ...
        if      (throwhsm.bouncestate == BOUNCE_IDLE)                   JoystickSwitchMsg(3, "Idle", &mavlink_bouncestate_msg);
        else if (throwhsm.bouncestate == BOUNCE_WAITBALL)               JoystickSwitchMsg(3, "WaitBall", &mavlink_bouncestate_msg);
        else if (throwhsm.bouncestate == BOUNCE_CATCHING)               JoystickSwitchMsg(3, "Catching", &mavlink_bouncestate_msg);
        else if (throwhsm.bouncestate == BOUNCE_GETTOPOSITION)          JoystickSwitchMsg(3, "GetToPositon", &mavlink_bouncestate_msg);
        else if (throwhsm.bouncestate == BOUNCE_READY)                  JoystickSwitchMsg(3, "Ready", &mavlink_bouncestate_msg);
        else if (throwhsm.bouncestate == BOUNCE_BOUNCE)                 JoystickSwitchMsg(3, "Bounce", &mavlink_bouncestate_msg);
        else if (throwhsm.bouncestate == BOUNCE_CATCHANDADJUSTPOSTURE)  JoystickSwitchMsg(3, "CatchAndAdjustPosture", &mavlink_bouncestate_msg);

        if      (throwhsm.throwstate == THROW_IDLE)                     JoystickSwitchMsg(4, "Idle", &mavlink_throwstate_msg);
        else if (throwhsm.throwstate == THROW_GATHERSTRENGTH)           JoystickSwitchMsg(4, "GatherStrength", &mavlink_throwstate_msg);
        else if (throwhsm.throwstate == THROW_CLEARBRACE)               JoystickSwitchMsg(4, "ClearBrace", &mavlink_throwstate_msg);
        else if (throwhsm.throwstate == THROW_ACCELERATE)               JoystickSwitchMsg(4, "Acclerate", &mavlink_throwstate_msg);
        else if (throwhsm.throwstate == THROW_THROWOUT)                 JoystickSwitchMsg(4, "ThrowOut", &mavlink_throwstate_msg);
        else if (throwhsm.throwstate == THROW_BACK)                     JoystickSwitchMsg(4, "Back", &mavlink_throwstate_msg);
        else if (throwhsm.throwstate == THROW_CATCHING)                 JoystickSwitchMsg(4, "Catching", &mavlink_throwstate_msg);
